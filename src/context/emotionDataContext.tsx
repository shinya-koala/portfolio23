...
export const EmotionDataContext = createContext<EmotionDataContextValue | undefined>(undefined);
...
export const EmotionDataProvider = ({ children }: { children: ReactNode }) => {
  const [emotionDataList, setEmotionDataList] = useState<IUserEmotions[]>([]);
	...
	const addUserEmotion = useCallback((userName: string, emotion: IEmotion) => {
		const newEmotionDataList = [...emotionDataList];
		const userEmotion = Object.values(newEmotionDataList).find(item => item.userName === userName);
		if (userEmotion) {
